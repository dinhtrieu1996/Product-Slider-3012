      ...
      .each(function () {
        ...
        if (preloader) {
          ...
          siblings_preload.animate({
            ...
            siblings_preload.parents('.simesy-slider-section').removeClass('preloader_style')
          });
        }
    })
...
var class_preload = pre_loader ? 'preloader_style' : '';
    if (config.slider_title) {
      if (config.slider_title_link) {
        html += '<h2 class="simesy-section-title '+class_preload+'"><a href="'+config.slider_title_link+'">'+config.title+'</a></h2>';
      } else {
    ...
    if (pre_loader) {
      html +=
        ...
        id +
        '" style=""><img src="https://cdn.shopify.com/s/files/1/0602/9449/6431/files/preloader.gif"/></div>';
    }
    ...
    if (config.filter_products === "collection") {
      $.each(list_products, function (index, value) {
        var avai = value.status.toLowerCase() == "active" ? true : false;
        if(avai){
          data_all.push({
            status: avai,
            available: value.totalInventory,
            image: value.featuredImage.originalSrc,
            url: "/products/" + value.handle,
            title: value.title,
            description: value.descriptionHtml,
            price: value.priceRangeV2.minVariantPrice.amount,
            compare_price: value.priceRangeV2.maxVariantPrice.amount,
            id: value.id.replace("gid://shopify/Product/", ""),
            variant: value.totalVariants,
          });
        }
      });
    } else {
    ...
    $.each(data_all, function (index, product_data) {
      ...
      if (product_image) {
        ...
        if (theme_style != "theme_twenty_two") {
          ...
          if (sale_ribbon && parseInt(product_data.compare_price) > parseInt(product_data.price)) {
            html_img +=
      ...
      if (pro_price) {
        ...
        if (parseInt(product_data.compare_price) > parseInt(product_data.price)) {
          html_price +=
        ...
        html_price +=
          '<span class="amout">' + formatMoney(product_data.price) + "</span>";
        html_price += "</div>";
      }
      if (pro_des) {
      ...
      if (theme_style == "theme_three") {
        ...
        if (add_to_cart_button) {
          ...
          html_theme +=
           ...
           html_theme +=
            '<a href="'+product_data.url+'" class="button add-to-cart btn-readmore">Read more</a>';
          }
          html_theme += "</p></div>";
        }
        html_theme += "</div>";
